...
window.tagcloud = (function(win, doc) { // ns
    ...
    function TagCloud (options) {
        ...
        self.config.radius = $("#card-tag .cl-card-content").width() / 2;
        ...
        var itemCandidates = self._getItems();
        self.items = itemCandidates.items;
        self.candidates = itemCandidates.candidates;
        ...
        for (var j = 0, len = self.candidates.length; j < len; j++) {
            self.candidates[j].element.index=j;

            //鼠标移出子元素,当前元素静止放大
            self.candidates[j].element.onmouseover = function(){
                self.index = this.index;
            };

            //鼠标移出子元素,当前元素继续滚动
            self.candidates[j].element.onmouseout = function(){
                self.index = -1;
            };
        }
        ...
        for (var j = 0, len = self.candidates.length; j < len; j++) {
            self.candidates[j].element.style.position = "absolute";
            self.candidates[j].element.style.zIndex = self.items.length + j + 1;
        }
    ...
    TagCloud.prototype = {
        ...
        update: function () {
            ...
            for (var j = 0, len = self.items.length; j < len; j++) {
                self.items[j].element.style.display = "block";
                ...
                if(self.index==self.items[j].element.index){

                    self.items[j].scale = 1.2; //取值范围0.6 ~ 3
                    self.items[j].fontsize = 24;
                    ...
                    // 位置偏右下了，优化一下
                    self.items[j].element.style.left = self.items[j].x + (self.box.offsetWidth - self.items[j].offsetWidth) / 2  - self.radius * 0.2 + "px";
                    self.items[j].element.style.top = self.items[j].y + (self.box.offsetHeight - self.items[j].offsetHeight) / 2 - self.radius * 0.2 + "px";
                }
                ...
                self.now = new Date();
                if(self.candidates.length && self.now.getTime() - self.date.getTime() > 2000){
                    self.date = self.now;
                    if(self.items.length < 20){
                        self.items.unshift(self.candidates.shift());
                    }else{
                        self.items[(j + 1) % self.items.length].element.style.display = "none";
                        self.candidates.push(self.items[(j + 1) % self.items.length]);
                        self.items.splice((j + 1) % self.items.length, 1);
                        self.items.unshift(self.candidates.shift());
                    }
        ...
        _getItems: function () {
            ...
            for (var i = 0; i < length; i++) {
                ...
                if(length > 20 && items.length < 20 && Math.random() < 10 / length){
                    items.push(item);
                }else{
            ...
            return {items: items, candidates: candidates};   //单元素数组
        }
