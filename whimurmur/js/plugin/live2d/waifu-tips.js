...
function showMessageLive2D(text, timeout, flag) {
...
function loadTipsMessage(result) {
    ...
    $.each(result.click, function (index, tips){
        $(document).on("click", tips.selector, function (){
            var text = getRandText(tips.text);
            text = text.render({text: $(this).text()});
            showMessageLive2D(text, 3000, true);
        });
    });
    $.each(result.seasons, function (index, tips){
        ...
        if((after.split('/')[0] <= now.getMonth()+1 && now.getMonth()+1 <= before.split('/')[0]) && 
           (after.split('/')[1] <= now.getDate() && now.getDate() <= before.split('/')[1])){
            ...
            showMessageLive2D(text, 6000, true);
        }
    });
    
    if (live2d_settings.showF12OpenMsg) {
        re.toString = function() {
            showMessageLive2D(getRandText(result.waifu.console_open_msg), 5000, true);
            return '';
        };
    }
    ...
    if (live2d_settings.showCopyMessage) {
        $(document).on('copy', function() {
            showMessageLive2D(getRandText(result.waifu.copy_message), 5000, true);
        });
    }
    
    $('.waifu-tool .fui-photo').click(function(){
        showMessageLive2D(getRandText(result.waifu.screenshot_message), 5000, true);
    ...
    $('.waifu-tool .fui-cross').click(function(){
        ...
        showMessageLive2D(getRandText(result.waifu.hidden_message), 1300, true);
    ...
    function loadOtherModel() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                showMessageLive2D(message, 3000, true);
            }
        });
    }
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.textures['id'] == 1 && (modelTexturesId == 1 || modelTexturesId == 0))
                    showMessageLive2D(waifu_tips.load_rand_textures[0], 3000, true);
                else showMessageLive2D(waifu_tips.load_rand_textures[1], 3000, true);
    	    ...
    	    case 'lwl12.com':
    	        $.getJSON('https://api.lwl12.com/hitokoto/v1?encode=realjson',function(result){
        	        if (!empty(result.source)) {
                        ...
                        window.setTimeout(function() {showMessageLive2D(text+waifu_tips.hitokoto_api_message['lwl12.com'][2], 3000, true);}, 5000);
                    } showMessageLive2D(result.text, 5000, true);
                });break;
    	    case 'fghrsh.net':
    	        $.getJSON('https://api.fghrsh.net/hitokoto/rand/?encode=jsc&uid=3335',function(result){
            	    if (!empty(result.source)) {
                        ...
                        window.setTimeout(function() {showMessageLive2D(text, 3000, true);}, 5000);
                        showMessageLive2D(result.hitokoto, 5000, true);
            	    }
                });break;
            case 'jinrishici.com':
                $.ajax({
                    ...
                    success: function (result, status) {
                        if (!empty(result.data.origin.title)) {
                            ...
                            window.setTimeout(function() {showMessageLive2D(text, 3000, true);}, 5000);
                        } showMessageLive2D(result.data.content, 5000, true);
                    }
                });break;
    	    default:
