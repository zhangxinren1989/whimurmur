...
function initCustomPage(){
    ...
    initTheme();
    initFont();
    initFunctinButtonMore();
}

function initFunctinButtonMore(){
    // 没有侧边栏和页内搜索时不显示搜索按钮
    if(!$("#sidebar #card-search").length){
        $("#rightBarAppendix").remove();
    }
    $("#function-more").on("click", function(){
       if($(this).hasClass("more-in")){
           $(this).removeClass("more-in");
           $(this).addClass("more-out");
           $("#function-button-more").toggleClass("function-button-more-show");
           $("#function-button-more").toggleClass("function-button-more-hidden");
       }else{
           $(this).removeClass("more-out");
           $(this).addClass("more-in");
           $("#function-button-more").toggleClass("function-button-more-show");
           $("#function-button-more").toggleClass("function-button-more-hidden");
       }
...
// 想抄一下csdn的layout，有些问题，先注掉
function initLayout(){
    clLeft = $(".cl-left");
    clRight = $(".cl-right");
    clHeader = $(".cl-header");
    clFooter = $(".cl-footer");
    clLeftH = clLeft.offset().top;
    clRightH = clRight.offset().top;
    clHeaderH = clHeader.outerHeight(true);
    clFooterH = clFooter.outerHeight(true);
    clFooterOff = clFooter.offset().top;
    leftF = false;
    rightF = false;
    $('.cl-left>*').each(function(i, item){
       clLeftH += $(item).outerHeight(true);
    });
    $('.cl-right>*').each(function(i, item){
       clRightH += $(item).outerHeight(true);
    });
    
    winH = $(window).height();
    $(window).scroll(function(e){
        var winScrollTop = $(window).scrollTop();
        if(winScrollTop + winH > clLeftH + clHeaderH){
            if(!leftF){
                clLeft.css('position', 'relative');
                leftF = true;
            }
            clLeft.css('top', (winScrollTop + winH - clHeaderH - clLeftH) + 'px');
        }else if(winScrollTop + winH < clLeftH + clHeaderH && leftF){
            clLeft.css('position', 'static');
            leftF = false;
        }
        
        if(winScrollTop + winH > clRightH + clHeaderH && !rightF){
            if(!rightF){
                clRight.css('position', 'relative');
                rightF = true;
            }
            clRight.css('top', (winScrollTop + winH - clHeaderH - clRightH) + 'px');
        }else if(winScrollTop + winH < clRightH + clHeaderH && rightF){
            clRight.css('position', 'static');
            rightF = false;
        }
    });
}
...
function initUser(){
    ...
    $.ajax({
    url: '/infoShare/loginUser/getLoginUserInfo',
    data: null,
    type: 'post',
    async: false,
    success: function(user){
        ...
        if(user && user.userName){
            ...
            window.userAvatar = user.userAvatar || (tPath + 'img/avatar.png');
        }
    ...
    $(".theme").click(function(e){
        ...
        if(changeTheme == $.cookie()["curTheme"]){
            ...
            /*setTimeout(function(){
                ...
                $("#theme-css").attr("href", "/templates/whimurmur/bootstrap/css/theme/bootstrap." + changeTheme + ".min.css");
...
function initDown(){
    ...
    if($(document).scrollTop() + winHeight < bodyHeight - 50){
        $("#down").show();
    }
    // 手机端使用body，电脑端使用html
    $("#down").click(function(){
...
        if($(document).scrollTop() + winHeight < bodyHeight - 50){  //距离顶部大于100px时
            $("#down").show();
        }else{
            $("#down").hide();
        }
